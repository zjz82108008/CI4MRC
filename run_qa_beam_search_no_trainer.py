...
# from models.pytorch_xlnet.modeling_xlnet_token import XLNetForQuestionAnswering, XLNetForQuestionAnsweringSimple
# from models.pytorch_xlnet.modeling_xlnet_split import XLNetForQuestionAnswering  ####
from models.pytorch_xlnet.modeling_xlnet_all import XLNetForQuestionAnswering  ####
...
def parse_args():
    ...
    # args.train_file = 'dataset/test.json'
    # args.validation_file = 'dataset/test.json'
    args.model_name_or_path = "xlnet-large-cased"
    args.output_dir = "output_squad_xlnet_large_token"
...
def main():
    ...
    ####
    # bbb = model.state_dict()
    update_dict = {}
    for i in range(24):
        state = torch.load('output_squad_xlnet_large/split/gp_split/transformer.layer.{}.ff.layer_1.weight_input_compl'.format(i))
        aaa = state.state_dict()
        for item in aaa.keys():
            update_dict['transformer.layer.{}.ff.split_mlp.{}.weight'.format(i, item[0])] = aaa[item]
    model.load_state_dict(update_dict, strict = False)
    ####
    for layer_idx, layer in enumerate(model.transformer.layer):
        labels = torch.load('output_squad_xlnet_large/split/gp_split/transformer.layer.{}.ff.layer_1.weight'.format(layer_idx))
        cluster_num = max(labels) + 1
        patterns = []
        for i in range(cluster_num):
            patterns.append(np.array(labels) == i)
        layer.ff.patterns = torch.Tensor(patterns).cuda()
    ####
    ...
    '''{
        "params": [p for n, p in model.named_parameters() if
                   not (any(nd in n for nd in no_decay) or ("split_mlp" in n))],  ####
        "weight_decay": args.weight_decay,
    },'''
