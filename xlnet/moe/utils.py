...
class MLPCenter(BlockCenter):
    ...
    def cal_center(self):
        ...
        for epoch in range(30):
            ...
            for i in range(hiddens.shape[0] // 100 * 98, hiddens.shape[0], 512):
            ...
            # for i in range(0, 512, 512):
                with torch.no_grad():
                    ...
                    scores = torch.matmul(acts, patterns) # 512, num_blocks, vary from 0 to 1
                    mask, labels = torch.topk(scores, k=int(num_blocks*0.9), dim=-1)
                    mask = mask > 0
                    
                    pred = model(input)
                    pred = torch.topk(pred, k=int(num_blocks*0.9), dim=-1)[1]
